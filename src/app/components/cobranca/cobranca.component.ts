...
export class CobrancaComponent implements OnInit {
  ...
  calcularDiasAtraso(emprestimo: Emprestimo): number {
    ...
    const venc = this.normalizeDate(emprestimo.proximoVencimento);
  ...
  private buildWhatsAppMessage(emprestimo: Emprestimo): string {
    ...
    if (this.normalizeDate(emprestimo.proximoVencimento) < hoje) {
    ...
    const diff = Math.ceil((this.normalizeDate(emprestimo.proximoVencimento).getTime() - hoje.getTime())/(1000*60*60*24));
